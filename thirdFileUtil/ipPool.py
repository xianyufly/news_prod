...
import sys
sys.path.append("..")
from models.TIpPool import TIpPool
import env
#系统变量
_env=env.initEnv()
...
def findip(type,pagenum,targeturl,path): # ip类型,页码,目标url,存放ip的路径
    ...
    for i in all:
        ...
        if is_avail == True:
            ...
            num=session.query(TIpPool).filter(TIpPool.ip==t[1].text).count()
            if num == 0 :
                #Ip 和端口保存到数据库
                ipPool=TIpPool(ip=t[1].text,port=t[2].text)
                session.add(ipPool)
                session.commit()
            # print(ip)
